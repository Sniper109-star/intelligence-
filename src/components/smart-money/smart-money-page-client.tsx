          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-border bg-surface-hover text-xs uppercase text-foreground-muted">
              <tr>
                ...
                <th className="px-3 py-3 whitespace-nowrap">Win Rate</th>
                <th className="px-3 py-3 whitespace-nowrap">Total Profit</th>
                <th className="px-3 py-3 whitespace-nowrap">Trades</th>
                <th className="px-3 py-3 whitespace-nowrap">Avg Trade (%)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {wallets.map((w, idx) => (
                <tr key={idx} className="hover:bg-surface-hover">
                  <td className="px-3 py-3 font-medium text-foreground">{w.category ? `${w.category}${idx + 1}` : `#${idx + 1}`}</td>
                  <td className="px-3 py-3 font-mono text-foreground">{w.address}</td>
                  ...
                  <td className="px-3 py-3 text-foreground">${w.totalProfit.toLocaleString()}</td>
                  <td className="px-3 py-3 text-foreground">{w.totalTrades}</td>
                  <td className="px-3 py-3 text-foreground">{(w.totalProfit / Math.max(w.totalTrades, 1)).toFixed(2)}%</td>
                </tr>
              ))}
            </tbody>
          </table>
